...
data_alles = ChemLogReader()
df = data_alles.chemical_log_data
df = df.dropna()
...
writer = pd.ExcelWriter(df_path + '/scores_chem_log_pca.xlsx')
...
writer = pd.ExcelWriter(df_path + '/loadings_chemicals_log_pca.xlsx')
...
writer = pd.ExcelWriter(df_path + '/expla-var_chemicals_log_pca.xlsx')
...
# 3D Plot with the localization of the parameters regarding the PC on a plane.
